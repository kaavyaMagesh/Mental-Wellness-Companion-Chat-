function injectStyles() {
  ...
  style.textContent = `
    ...
    .range-track{position:absolute;inset:0;border-radius:999px;background:linear-gradient(90deg,#c83c28 50%,#5f8a4a 83%,#ede9df 83%);}
...
export function renderSelfEsteemForm() {
  injectStyles();
  ...
  const optionLabels = [
    { label: "Strongly Agree", score: 3, tag: "High" },
    { label: "Agree", score: 2, tag: "Moderate" },
    { label: "Disagree", score: 1, tag: "Low" },
    { label: "Strongly Disagree", score: 0, tag: "None" }
  ];
  ...
  try {
    const s = localStorage.getItem('selfesteem_answers');
    const q = localStorage.getItem('selfesteem_current');
        ...
        <div class="max-w-2xl mx-auto w-full px-4 py-8 sm:px-6 sm:py-12">
          ...
          <div id="quizView" class="space-y-6">
            <div class="flex items-center justify-between border-b border-cream-border pb-4">
              <div>
                <h2 class="font-serif text-2xl text-warm-dark leading-tight">Self-Esteem Assessment</h2>
                <p class="text-xs text-warm-gray mt-1">Rosenberg Self-Esteem Scale (RSES)</p>
              </div>
            ...
            <div class="card p-5 sm:p-6 mb-6" id="questionCard">
              ...
              <h3 class="font-serif text-lg sm:text-xl text-warm-dark leading-snug mb-6" id="questionText">On the whole, I am satisfied with myself.</h3>
          ...
          <div id="completionView" class="hidden text-center space-y-6 py-12">
            <div class="card p-8 sm:p-10 text-center">
              ...
              <p class="text-sm text-warm-gray mb-8">Thank you for completing the Self-Esteem Assessment. Your results are ready.</p>
          ...
          <div id="resultsView" class="hidden w-full space-y-6">
            <div class="card p-6 sm:p-8 text-center flex flex-col items-center">
              <p class="text-[11px] font-medium text-warm-gray uppercase tracking-wider mb-4">Your Self-Esteem Score</p>
              <div class="relative w-36 h-36 mb-4">
                ...
                <div class="absolute inset-0 flex flex-col items-center justify-center">
                  ...
                  <span class="text-xs text-warm-gray">/ 30</span>
                </div>
              </div>
              <span class="badge badge-low mb-3" id="severityBadge">Normal Range</span>
              ...
              <div class="mt-5 w-full max-w-xs">
                ...
                <div class="flex justify-between text-[10px] text-warm-gray mt-1">
                  <span>Low Self-Esteem (<15)</span><span>Normal (15–25)</span><span>High (26+)</span>
                </div>
              </div>
            </div>
            ...
            <div class="card p-5 sm:p-6">
              ...
              <div id="chartRadar" class="hidden">
                <p class="text-xs text-warm-gray mb-3">Visualisation across self-perception dimensions — outer edge = max (15).</p>
                <canvas id="radarCanvas" width="420" height="300" style="max-width:100%;margin:0 auto;"></canvas>
              </div>

              <div id="chartDist" class="hidden">
                <p class="text-xs text-warm-gray mb-3">Approximate population distribution — where your score sits.</p>
            ...
            <div class="card p-5 sm:p-6">
              ...
              <p class="text-xs text-warm-gray mb-4">RSES measures Self-Competence and Self-Liking.</p>
            ...
            <div class="card p-5 sm:p-6 mb-5">
              ...
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-2">
                <div class="resource-item">
                  ...
                  <div><p class="text-sm font-medium text-warm-dark">Self-Compassion Handbook</p><p class="text-xs text-warm-gray">Mindfulness and self-kindness exercises</p></div>
                </div>
                <div class="resource-item">
                  <div class="resource-icon">
                  ...
                  <div><p class="text-sm font-medium text-warm-dark">Core Beliefs Worksheet</p><p class="text-xs text-warm-gray">Overcoming cognitive biases and inner criticism</p></div>
                </div>
              </div>
            </div>
            ...
            <div class="card p-5 mb-5" style="background:rgba(232,101,26,.04);">
              <div class="flex gap-3 items-start">
                ...
                <div>
                  <p class="text-xs font-medium text-warm-dark mb-1">About the RSES</p>
                  <p class="text-xs text-warm-gray leading-relaxed">The Rosenberg Self-Esteem Scale (Rosenberg, 1965) is a standard psychological instrument for self-esteem evaluation. It is not diagnostic, but rather reflective of core self-perception.</p>
                </div>
              </div>
            </div>
  ...
  restartBtn.onclick = () => {
    try { localStorage.removeItem('selfesteem_answers'); localStorage.removeItem('selfesteem_current'); } catch (e) {}
    renderSelfEsteemForm();
  };
  ...
  function renderQuestion() {
    ...
    optionLabels.forEach((opt, idx) => {
      const scoreValue = questions[currentQuestion].reverse ? (3 - opt.score) : opt.score;
      ...
      div.onclick = () => {
        ...
        try {
          localStorage.setItem('selfesteem_answers', JSON.stringify(answers));
          localStorage.setItem('selfesteem_current', String(currentQuestion));
        } catch(e){}
        renderQuestion();
      };
      div.innerHTML = `
  ...
  function getDomainScores() {
    // Self-competence: Q1,3,4,7,10
    const competence = answers[0] + answers[2] + answers[3] + answers[6] + answers[9];
    // Self-liking: Q2,5,6,8,9
    const liking = answers[1] + answers[4] + answers[5] + answers[7] + answers[8];
    return { competence, liking };
  }
  ...
  function showResults() {
    ...
    const offset = circ - (score / 30) * circ;
    ...
    if (score < 15) {
      ...
      badge.innerHTML = `<svg width="10" height="10" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M12 9v4M12 16h.01"/></svg> Low Self-Esteem`;
      text.textContent = 'Your score suggests lower levels of self-confidence or self-worth. Cultivating self-compassion can help build resilience.';
      ring.style.stroke = '#c83c28';
      setTimeout(() => { document.getElementById('rangeMarker').style.left = (score/15)*50 + '%'; }, 200);
    } else if (score <= 25) {
      level = 'normal';
      badge.className = 'badge badge-low mb-3';
      badge.innerHTML = `<svg width="10" height="10" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7"/></svg> Normal Range`;
      text.textContent = 'Your responses reflect a healthy, typical level of self-acceptance and confidence.';
      ring.style.stroke = '#5f8a4a';
      setTimeout(() => { document.getElementById('rangeMarker').style.left = (50 + ((score-15)/10)*33) + '%'; }, 200);
    } else {
      level = 'high';
      badge.className = 'badge badge-low mb-3';
      badge.innerHTML = `<svg width="10" height="10" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7"/></svg> High Self-Esteem`;
      text.textContent = 'You show strong, positive self-esteem and a high degree of confidence and self-satisfaction.';
      ring.style.stroke = '#5f8a4a';
      setTimeout(() => { document.getElementById('rangeMarker').style.left = (83 + ((score-26)/4)*14) + '%'; }, 200);
    }
  ...
  function buildDomainCards() {
    const { competence, liking } = getDomainScores();
    document.getElementById('domainCards').innerHTML = `
      <div class="card p-4" style="background:rgba(95,138,74,.04);">
        <p class="text-xs font-medium text-warm-dark">Self-Competence</p>
        <p class="font-serif text-2xl text-warm-dark mt-1">${competence}<span class="text-xs text-warm-gray">/15</span></p>
        <div class="bar-bg mt-2"><div class="bar-fill" style="width:${(competence/15)*100}%;background:#5f8a4a;"></div></div>
      </div>
      <div class="card p-4" style="background:rgba(232,101,26,.04);">
        <p class="text-xs font-medium text-warm-dark">Self-Liking</p>
        <p class="font-serif text-2xl text-warm-dark mt-1">${liking}<span class="text-xs text-warm-gray">/15</span></p>
        <div class="bar-bg mt-2"><div class="bar-fill" style="width:${(liking/15)*100}%;background:#e8651a;"></div></div>
      </div>
    `;
  }
  ...
  function buildInsights(score) {
    ...
    const { competence, liking } = getDomainScores();
    if (competence <= 7) {
      ins.push({ icon: `💪`, bg: 'rgba(232,101,26,.1)', col: '#e8651a', text: "Your self-competence score is low. Try setting small, achievable daily goals to build confidence in your capabilities." });
    }
    if (liking <= 7) {
      ins.push({ icon: `❤️`, bg: 'rgba(200,60,40,.1)', col: '#c83c28', text: "Self-liking indicators are low. Focus on practicing self-compassion exercises and addressing self-criticism." });
    }
    if (ins.length === 0) {
      ins.push({ icon: `✨`, bg: 'rgba(95,138,74,.1)', col: '#5f8a4a', text: "Your self-perception profile is balanced and healthy." });
    }
  ...
  function buildResponseReview() {
    const shortQs = ["Self-satisfaction", "No good at all", "Good qualities", "Ability level", "Proud of", "Feel useless", "Person of worth", "Self-respect", "Failure inclination", "Positive attitude"];
    document.getElementById('responseReview').innerHTML = answers.map((raw, idx) => {
      const scoreVal = questions[idx].reverse ? (3 - raw) : raw;
      const opt = optionLabels.find(o => o.score === scoreVal) || { label: "N/A" };
      return `
  ...
  function buildDistChart(userScore) {
    ...
    [0, 10, 15, 20, 25, 30].forEach(s => {
      const x=pad.left+(s/30)*cW;
      ctx.fillText(s, x, H-6);
    });
    const ux=pad.left+(userScore/30)*cW;
  ...
  function buildRadarChart() {
    ...
    const { competence, liking } = getDomainScores();
    const domains = [
      { label: "Competence", val: competence, max: 15 },
      { label: "Self-Liking", val: liking, max: 15 }
    ];
  ...
  function buildBarChart() {
    ...
    answers.forEach((raw, idx) => {
      const pct = (raw / 3) * 100;
      const cols = ['#ede9df', '#c8b878', '#e8651a', '#5f8a4a'];
      ...
      row.innerHTML = `
        <div class="flex items-center justify-between mb-1">
          ...
          <span style="font-size:11px;font-family:'JetBrains Mono',monospace;color:#2a2520;">${raw}/3</span>
        </div>
