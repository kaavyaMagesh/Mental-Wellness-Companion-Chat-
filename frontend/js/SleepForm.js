function injectStyles() {
  ...
  style.textContent = `
    ...
    .range-track{position:absolute;inset:0;border-radius:999px;background:linear-gradient(90deg,#c83c28 50%,#e8651a 75%,#5f8a4a 100%);}
...
export function renderSleepForm() {
  injectStyles();
  const app = document.getElementById("app");

  const questions = [
    { text: "Thinking about a typical night in the last month, how long did it usually take you to fall asleep?", type: "time" },
    { text: "Thinking about a typical night in the last month, if you woke up during the night, how long were you awake for in total?", type: "time" },
    { text: "In the last month, how many nights a week did you have a problem with your sleep?", type: "nights" },
    { text: "Over the last month, how would you rate your typical night's sleep?", type: "quality" },
    { text: "To what extent has poor sleep affected your mood, energy, or relationships in the last month?", type: "impact" },
    { text: "To what extent has poor sleep affected your concentration, productivity, or ability to stay awake in the last month?", type: "impact" },
    { text: "How much has poor sleep troubled you in general?", type: "impact" },
    { text: "How long have you had a problem with your sleep?", type: "duration" }
  ];

  const optionScales = {
    time: [
      { label: "0–15 min", score: 4, tag: "Excellent" },
      { label: "16–30 min", score: 3, tag: "Good" },
      { label: "31–45 min", score: 2, tag: "Moderate" },
      { label: "46–60 min", score: 1, tag: "Poor" },
      { label: ">60 min", score: 0, tag: "Severe" }
    ],
    nights: [
      { label: "0–1 nights", score: 4, tag: "Rare" },
      { label: "2 nights", score: 3, tag: "Low" },
      { label: "3 nights", score: 2, tag: "Moderate" },
      { label: "4 nights", score: 1, tag: "Frequent" },
      { label: "5–7 nights", score: 0, tag: "Chronic" }
    ],
    quality: [
      { label: "Very good", score: 4, tag: "Excellent" },
      { label: "Good", score: 3, tag: "Good" },
      { label: "Fair", score: 2, tag: "Moderate" },
      { label: "Poor", score: 1, tag: "Poor" },
      { label: "Very poor", score: 0, tag: "Severe" }
    ],
    impact: [
      { label: "Not at all", score: 4, tag: "None" },
      { label: "A little", score: 3, tag: "Minimal" },
      { label: "Moderately", score: 2, tag: "Moderate" },
      { label: "Quite a bit", score: 1, tag: "Significant" },
      { label: "Extremely", score: 0, tag: "Severe" }
    ],
    duration: [
      { label: "No problem / <1 month", score: 4, tag: "Acute" },
      { label: "1–2 months", score: 3, tag: "Short term" },
      { label: "3–6 months", score: 2, tag: "Medium term" },
      { label: "6–12 months", score: 1, tag: "Chronic" },
      { label: "More than a year", score: 0, tag: "Long term" }
    ]
  };
  ...
  try {
    const s = localStorage.getItem('sleep_answers');
    const q = localStorage.getItem('sleep_current');
        ...
        <div class="max-w-2xl mx-auto w-full px-4 py-8 sm:px-6 sm:py-12">
          ...
          <div id="quizView" class="space-y-6">
            <div class="flex items-center justify-between border-b border-cream-border pb-4">
              <div>
                <h2 class="font-serif text-2xl text-warm-dark leading-tight">Sleep Quality (SCI)</h2>
                <p class="text-xs text-warm-gray mt-1">Sleep Condition Indicator Assessment</p>
              </div>
              <div class="text-right">
                <span class="text-xs font-medium text-warm-gray" id="questionCounter">Question 1 of 8</span>
                <div class="flex items-center gap-2 mt-1">
                  <div class="progress-track w-24"><div class="progress-fill" id="progressFill" style="width: 12.5%;"></div></div>
                  <span class="text-[10px] font-mono font-medium text-warm-dark" id="progressPercent">12.5%</span>
                </div>
              </div>
            </div>
            ...
            <div class="card p-5 sm:p-6 mb-6" id="questionCard">
              ...
              <h3 class="font-serif text-lg sm:text-xl text-warm-dark leading-snug mb-6" id="questionText">How long does it take you to fall asleep?</h3>
          ...
          <div id="completionView" class="hidden text-center space-y-6 py-12">
            <div class="card p-8 sm:p-10 text-center">
              ...
              <p class="text-sm text-warm-gray mb-8">Thank you for completing the Sleep Quality Assessment. Your results are ready.</p>
          ...
          <div id="resultsView" class="hidden w-full space-y-6">
            <div class="card p-6 sm:p-8 text-center flex flex-col items-center">
              <p class="text-[11px] font-medium text-warm-gray uppercase tracking-wider mb-4">Your Sleep Quality Score</p>
              <div class="relative w-36 h-36 mb-4">
                ...
                <div class="absolute inset-0 flex flex-col items-center justify-center">
                  ...
                  <span class="text-xs text-warm-gray">/ 32</span>
                </div>
              </div>
              <span class="badge badge-low mb-3" id="severityBadge">Good Sleep</span>
              ...
              <div class="mt-5 w-full max-w-xs">
                ...
                <div class="flex justify-between text-[10px] text-warm-gray mt-1">
                  <span>Insomnia Risk (0–16)</span><span>Mild Difficulties (17-24)</span><span>Good Sleep (25–32)</span>
                </div>
              </div>
            </div>
            ...
            <div class="card p-5 sm:p-6">
              ...
              <div id="chartRadar" class="hidden">
                <p class="text-xs text-warm-gray mb-3">Visualisation across sleep domains — outer edge = maximum quality score (4).</p>
                <canvas id="radarCanvas" width="420" height="300" style="max-width:100%;margin:0 auto;"></canvas>
              </div>

              <div id="chartDist" class="hidden">
                <p class="text-xs text-warm-gray mb-3">Approximate SCI adult population distribution — where your score sits.</p>
            ...
            <div class="card p-5 sm:p-6">
              ...
              <p class="text-xs text-warm-gray mb-4">SCI measures sleep continuity, daytime impact, and perceived severity.</p>
            ...
            <div class="card p-5 sm:p-6 mb-5">
              ...
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-2">
                <div class="resource-item">
                  ...
                  <div><p class="text-sm font-medium text-warm-dark">Sleep Hygiene Checklist</p><p class="text-xs text-warm-gray">Quick checks to optimize your sleep routine</p></div>
                </div>
                <div class="resource-item">
                  <div class="resource-icon">
                  ...
                  <div><p class="text-sm font-medium text-warm-dark">Relaxation Techniques</p><p class="text-xs text-warm-gray">Wind-down guidance and muscle relaxation</p></div>
                </div>
              </div>
            </div>
            ...
            <div class="card p-5 mb-5" style="background:rgba(232,101,26,.04);">
              <div class="flex gap-3 items-start">
                ...
                <div>
                  <p class="text-xs font-medium text-warm-dark mb-1">About the SCI</p>
                  <p class="text-xs text-warm-gray leading-relaxed">The Sleep Condition Indicator (SCI) is a validated tool for identifying insomnia risks and general sleep continuity, aligned with DSM-5 guidelines. This is a wellness indicator and not a diagnostic clinical decision. Consult a clinician for diagnostic support.</p>
                </div>
              </div>
            </div>
  ...
  restartBtn.onclick = () => {
    try { localStorage.removeItem('sleep_answers'); localStorage.removeItem('sleep_current'); } catch (e) {}
    renderSleepForm();
  };
  ...
  function renderQuestion() {
    ...
    scale.forEach((opt, idx) => {
      ...
      div.onclick = () => {
        ...
        try {
          localStorage.setItem('sleep_answers', JSON.stringify(answers));
          localStorage.setItem('sleep_current', String(currentQuestion));
        } catch(e){}
        renderQuestion();
      };
      div.innerHTML = `
  ...
  function getDomainScores() {
    const continuity = answers[0] + answers[1] + answers[2];
    const impact = answers[4] + answers[5];
    const severity = answers[3] + answers[6] + answers[7];
    return { continuity, impact, severity };
  }
  ...
  function showResults() {
    ...
    const offset = circ - (score / 32) * circ;
    ...
    if (score <= 16) {
      level = 'probable insomnia';
      badge.className = 'badge badge-high mb-3';
      badge.innerHTML = `<svg width="10" height="10" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M12 9v4M12 16h.01"/></svg> Probable Insomnia`;
      text.textContent = 'Your responses indicate significant sleep difficulties. Consider discussing these results with a healthcare professional.';
      ring.style.stroke = '#c83c28';
      setTimeout(() => { document.getElementById('rangeMarker').style.left = (score/16)*50 + '%'; }, 200);
    } else if (score <= 24) {
      level = 'mild difficulties';
      badge.className = 'badge badge-moderate mb-3';
      badge.innerHTML = `<svg width="10" height="10" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M12 9v4M12 16h.01"/></svg> Mild Sleep Difficulties`;
      text.textContent = 'You may have occasional disruptions or mild sleep quality issues. Small changes in bedroom environment or wind-down habits could help.';
      ring.style.stroke = '#e8651a';
      setTimeout(() => { document.getElementById('rangeMarker').style.left = (50 + ((score-17)/7)*25) + '%'; }, 200);
    } else {
      level = 'good sleep';
      badge.className = 'badge badge-low mb-3';
      badge.innerHTML = `<svg width="10" height="10" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7"/></svg> Good Sleep Quality`;
      text.textContent = 'Your sleep quality is in a healthy range. Maintain your current sleep schedules and bedtime routines.';
      ring.style.stroke = '#5f8a4a';
      setTimeout(() => { document.getElementById('rangeMarker').style.left = (75 + ((score-25)/7)*23) + '%'; }, 200);
    }
  ...
  function buildDomainCards() {
    const { continuity, impact, severity } = getDomainScores();
    document.getElementById('domainCards').innerHTML = `
      <div class="card p-4" style="background:rgba(95,138,74,.04);">
        <p class="text-xs font-medium text-warm-dark">Continuity</p>
        <p class="font-serif text-2xl text-warm-dark mt-1">${continuity}<span class="text-xs text-warm-gray">/12</span></p>
        <div class="bar-bg mt-2"><div class="bar-fill" style="width:${(continuity/12)*100}%;background:#5f8a4a;"></div></div>
      </div>
      <div class="card p-4" style="background:rgba(232,101,26,.04);">
        <p class="text-xs font-medium text-warm-dark">Daytime Impact</p>
        <p class="font-serif text-2xl text-warm-dark mt-1">${impact}<span class="text-xs text-warm-gray">/8</span></p>
        <div class="bar-bg mt-2"><div class="bar-fill" style="width:${(impact/8)*100}%;background:#e8651a;"></div></div>
      </div>
      <div class="card p-4" style="background:rgba(200,60,40,.04);">
        <p class="text-xs font-medium text-warm-dark">Perceived Severity</p>
        <p class="font-serif text-2xl text-warm-dark mt-1">${severity}<span class="text-xs text-warm-gray">/12</span></p>
        <div class="bar-bg mt-2"><div class="bar-fill" style="width:${(severity/12)*100}%;background:#c83c28;"></div></div>
      </div>
    `;
  }
  ...
  function buildInsights(score) {
    ...
    const { continuity, impact, severity } = getDomainScores();
    if (continuity <= 6) {
      ins.push({ icon: `🌙`, bg: 'rgba(232,101,26,.1)', col: '#e8651a', text: "Your sleep continuity is reduced. Set a consistent bedtime and limit blue-light exposure 1 hour before bed." });
    }
    if (impact <= 4) {
      ins.push({ icon: `☀️`, bg: 'rgba(200,60,40,.1)', col: '#c83c28', text: "Sleep issues are impacting your daytime productivity or mood. Regular physical activity can help deepen night sleep." });
    }
    if (severity <= 6) {
      ins.push({ icon: `🛌`, bg: 'rgba(200,60,40,.1)', col: '#c83c28', text: "High perceived sleep worry is present. Try progressive muscle relaxation exercises in bed." });
    }
    if (ins.length === 0) {
      ins.push({ icon: `✨`, bg: 'rgba(95,138,74,.1)', col: '#5f8a4a', text: "Excellent sleep habits! Keep regular wind-down patterns." });
    }
  ...
  function buildResponseReview() {
    const shortQs = ["Sleep latency", "Wake time", "Problem nights", "Quality rating", "Mood impact", "Concentration impact", "Troubled feelings", "Duration"];
  ...
  function buildDistChart(userScore) {
    ...
    [0, 10, 16, 24, 32].forEach(s => {
      const x=pad.left+(s/32)*cW;
      ctx.fillText(s, x, H-6);
    });
    const ux=pad.left+(userScore/32)*cW;
  ...
  function buildRadarChart() {
    ...
    const { continuity, impact, severity } = getDomainScores();
    const domains = [
      { label: "Continuity", val: continuity, max: 12 },
      { label: "Daytime Impact", val: impact, max: 8 },
      { label: "Severity", val: severity, max: 12 }
    ];
