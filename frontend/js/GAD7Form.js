...
export function renderGAD7Form() {
  injectStyles();
  ...
  try {
    const s = localStorage.getItem('gad7_answers');
    const q = localStorage.getItem('gad7_current');
      ...
      <main class="flex-1 overflow-y-auto bg-cream p-4 sm:p-8 md:p-12">
        <div class="max-w-2xl mx-auto">
          ...
          <div id="quizView" class="w-full flex flex-col">
            <div class="mb-6">
              <h1 class="font-serif text-2xl sm:text-3xl text-warm-dark mb-1">GAD-7 Anxiety Assessment</h1>
              <p class="text-sm text-warm-gray mb-5">Measure generalized anxiety levels and emotional tension over the past 2 weeks.</p>
              <div class="flex items-center justify-between mb-2">
                <span class="text-xs text-warm-gray" id="questionCounter">Question 1 of 7</span>
                <span class="text-xs font-mono text-warm-gray" id="progressPercent">14%</span>
              </div>
              <div class="progress-track"><div class="progress-fill" id="progressFill" style="width:14%;"></div></div>
            </div>

            <div class="card p-5 mb-5">
              <p class="text-sm font-medium text-warm-dark mb-1">During the last 2 weeks, how often have you been bothered by the following?</p>
          ...
          <div id="completionView" class="hidden w-full">
            <div class="card p-8 sm:p-10 text-center">
              ...
              <p class="text-sm text-warm-gray mb-8">Thank you for completing the GAD-7 Anxiety Assessment. Your results are ready.</p>
          ...
          <div id="resultsView" class="hidden w-full space-y-6">
            <div class="card p-6 sm:p-8 text-center flex flex-col items-center">
              <p class="text-[11px] font-medium text-warm-gray uppercase tracking-wider mb-4">Your Anxiety Score</p>
              <div class="relative w-36 h-36 mb-4">
                ...
                <div class="absolute inset-0 flex flex-col items-center justify-center">
                  ...
                  <span class="text-xs text-warm-gray">/ 21</span>
                </div>
              </div>
              <span class="badge badge-moderate mb-3" id="severityBadge">Minimal Anxiety</span>
              <p class="text-sm text-warm-gray leading-relaxed max-w-md" id="severityText"></p>

              <!-- Range bar -->
              <div class="mt-5 w-full max-w-xs">
                <div class="range-bar">
                  <div class="range-track" style="background: linear-gradient(90deg, #5f8a4a 24%, #e8651a 66%, #c83c28 100%);"></div>
                  <div class="range-marker" id="rangeMarker" style="left:50%;"></div>
                </div>
                <div class="flex justify-between text-[10px] text-warm-gray mt-1">
                  <span>Minimal (0–4)</span><span>Mild/Mod (5-14)</span><span>Severe (15–21)</span>
                </div>
              </div>
            </div>
            ...
            <div class="card p-5 sm:p-6">
              ...
              <div id="chartRadar" class="hidden">
                <p class="text-xs text-warm-gray mb-3">Visualisation across all 7 items — outer edge = maximum (3).</p>
                <canvas id="radarCanvas" width="420" height="300" style="max-width:100%;margin:0 auto;"></canvas>
              </div>

              <div id="chartDist" class="hidden">
                <p class="text-xs text-warm-gray mb-3">Approximate GAD-7 adult population distribution — where your score sits.</p>
            ...
            <div class="card p-5 sm:p-6">
              ...
              <p class="text-xs text-warm-gray mb-4">The GAD-7 measures two dimensions of anxiety expression.</p>
            ...
            <div class="card p-5 sm:p-6 mb-5">
              ...
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-2">
                <div class="resource-item">
                  ...
                  <div><p class="text-sm font-medium text-warm-dark">Anxiety Coping Skills</p><p class="text-xs text-warm-gray">Evidence-based CBT grounding techniques</p></div>
                </div>
                <div class="resource-item">
                  <div class="resource-icon">
                  ...
                  <div><p class="text-sm font-medium text-warm-dark">5-4-3-2-1 Grounding</p><p class="text-xs text-warm-gray">A sensory exercise to manage panic and anxiety</p></div>
                </div>
                <div class="resource-item">
                  <div class="resource-icon">
                  ...
                  <div><p class="text-sm font-medium text-warm-dark">Guided Calm Meditation</p><p class="text-xs text-warm-gray">Brief mindfulness sessions for acute anxiety</p></div>
                </div>
                <div class="resource-item sm:col-span-2">
                  ...
                  <div><p class="text-sm font-medium text-warm-dark">Emotional Regulation</p><p class="text-xs text-warm-gray">Strategies to navigate overwhelming feelings and build long-term calm</p></div>
                </div>
              </div>
            </div>
            ...
            <div class="card p-5 mb-5" style="background:rgba(232,101,26,.04);">
              <div class="flex gap-3 items-start">
                ...
                <div>
                  <p class="text-xs font-medium text-warm-dark mb-1">About the GAD-7</p>
                  <p class="text-xs text-warm-gray leading-relaxed">Developed by Spitzer, Kroenke, Williams &amp; Löwe (2006). This is a validated psychological tool for measuring anxiety symptoms, not a clinical diagnosis. Scores reflect the past two weeks. If you are concerned about your mental health, please consult a qualified professional.</p>
                </div>
              </div>
            </div>
  ...
  restartBtn.onclick = () => {
    try { localStorage.removeItem('gad7_answers'); localStorage.removeItem('gad7_current'); } catch (e) {}
    renderGAD7Form();
  };
    ...
    optionLabels.forEach((opt, idx) => {
      ...
      div.onclick = () => {
        ...
        try {
          ...
          localStorage.setItem('gad7_current', String(currentQuestion));
        } catch(e){}
        renderQuestion();
      };
      div.innerHTML = `
  ...
  function showResults() {
    ...
    const offset = circ - (score / 21) * circ;
    ...
    if (score <= 4) {
      ...
      badge.innerHTML = `<svg width="10" height="10" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7"/></svg> Minimal Anxiety`;
      text.textContent = 'Your responses suggest minimal anxiety levels. You seem to handle daily stressors effectively without significant emotional strain.';
      ring.style.stroke = '#5f8a4a';
      setTimeout(() => { document.getElementById('rangeMarker').style.left = Math.min(25, (score/4)*25)+'%'; }, 200);
    } else if (score <= 9) {
      level = 'mild';
      badge.className = 'badge badge-low mb-3';
      badge.innerHTML = `<svg width="10" height="10" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7"/></svg> Mild Anxiety`;
      text.textContent = 'You are experiencing mild anxiety symptoms. A slight baseline level of worry or tension is present but generally manageable.';
      ring.style.stroke = '#c8b878';
      setTimeout(() => { document.getElementById('rangeMarker').style.left = (25 + ((score-5)/5)*25)+'%'; }, 200);
    } else if (score <= 14) {
      level = 'moderate';
      badge.className = 'badge badge-moderate mb-3';
      badge.innerHTML = `<svg width="10" height="10" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M12 9v4M12 16h.01"/></svg> Moderate Anxiety`;
      text.textContent = 'Your responses indicate moderate anxiety levels. Tension, restlessness, and excessive worry might be affecting your daily routine.';
      ring.style.stroke = '#e8651a';
      setTimeout(() => { document.getElementById('rangeMarker').style.left = (50 + ((score-10)/5)*22)+'%'; }, 200);
    } else {
      level = 'severe';
      badge.className = 'badge badge-high mb-3';
      badge.innerHTML = `<svg width="10" height="10" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M12 9v4M12 16h.01"/></svg> Severe Anxiety`;
      text.textContent = 'Your responses suggest severe anxiety. Feelings of panic, excessive worry, and restlessness are likely impacting your ability to function.';
      ring.style.stroke = '#c83c28';
      setTimeout(() => { document.getElementById('rangeMarker').style.left = Math.min(97, 72+((score-15)/7)*25)+'%'; }, 200);
    }
    ...
    const gadHistory = JSON.parse(localStorage.getItem('assessment_history') || '[]');
    gadHistory.push(historyEntry);
    localStorage.setItem('assessment_history', JSON.stringify(gadHistory));
    // ─────────────────────────────────────────────────────────────────────
  ...
  function buildBarChart() {
    const shortQs = ["Nervousness", "Uncontrolled worry", "Excessive worry", "Trouble relaxing", "Restlessness", "Irritability", "Fear / dread"];
    ...
    document.getElementById('domainCards').innerHTML = `
      <div class="card p-4" style="background:rgba(232,101,26,.04);">
        <div class="flex items-center gap-2 mb-2">
          ...
          <p class="text-xs font-medium text-warm-dark">Worry &amp; Apprehension</p>
        </div>
        ...
        <p class="text-xs text-warm-gray mt-2">Fear, dread, uncontrolled &amp; excessive thoughts</p>
      </div>
      <div class="card p-4" style="background:rgba(200,60,40,.04);">
        <div class="flex items-center gap-2 mb-2">
          ...
          <p class="text-xs font-medium text-warm-dark">Tension &amp; Irritability</p>
        </div>
        ...
        <p class="text-xs text-warm-gray mt-2">Trouble relaxing, restlessness, and irritability</p>
      </div>
    `;
  }
  ...
  function buildInsights(score, level) {
    ...
    if ((answers[0] || 0) >= 2 || (answers[1] || 0) >= 2) {
      ins.push({icon:`<svg width="14" height="14" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><circle cx="12" cy="12" r="9"/><path stroke-linecap="round" stroke-linejoin="round" d="M12 9v4M12 16h.01"/></svg>`, bg:'rgba(232,101,26,.1)', col:'#e8651a', text:'Elevated levels of nervousness and uncontrollable worry. Practice cognitive reframing or journaling to release persistent worries.'});
    }
    if ((answers[3] || 0) >= 2 || (answers[4] || 0) >= 2) {
      ins.push({icon:`<svg width="14" height="14" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M13 10V3L4 14h7v7l9-11h-7z"/></svg>`, bg:'rgba(95,138,74,.12)', col:'#5f8a4a', text:'Difficulty relaxing and physical restlessness are present. Box breathing or progress muscle relaxation (PMR) can help soothe physical tension.'});
    }
    if ((answers[5] || 0) >= 2) {
      ins.push({icon:`<svg width="14" height="14" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10"/></svg>`, bg:'rgba(200,60,40,.1)', col:'#c83c28', text:'Becoming easily annoyed or irritable. Daily stress breaks or setting clear boundaries can prevent emotional exhaustion.'});
    }
    if (level === 'minimal') {
      ins.push({icon:`<svg width="14" height="14" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7"/></svg>`, bg:'rgba(95,138,74,.12)', col:'#5f8a4a', text:'Your anxiety levels are low. Keep maintaining regular self-care check-ins and supportive routines.'});
    }
  ...
  function buildResponseReview() {
    const shortQs = ["Nervous / anxious", "Uncontrolled worry", "Excessive worry", "Trouble relaxing", "Restlessness", "Irritability", "Fear / dread"];
  ...
  function buildDistChart(userScore) {
    ...
    [0, 4, 9, 14, 21].forEach(s => {
      const x=pad.left+(s/21)*cW;
    ...
    const ux=pad.left+(userScore/21)*cW;
  ...
  function buildRadarChart() {
    ...
    const axes=[
      {label:'Nervousness', idx:0},
      {label:'Uncontrolled worry', idx:1},
      {label:'Excessive worry', idx:2},
      {label:'Trouble relaxing', idx:3},
      {label:'Restlessness', idx:4},
      {label:'Irritability', idx:5},
      {label:'Fear / dread', idx:6}
    ];
    const n=axes.length;
