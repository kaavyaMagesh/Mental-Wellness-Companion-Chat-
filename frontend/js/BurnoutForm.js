function injectStyles() {
  ...
  style.textContent = `
    ...
    .range-track{position:absolute;inset:0;border-radius:999px;background:linear-gradient(90deg,#5f8a4a 24%,#ede9df 24%,#ede9df 49%,#e8651a 49%,#e8651a 74%,#c83c28 74%);}
...
export function renderBurnoutForm() {
  injectStyles();
  const app = document.getElementById("app");

  const questions = [
    { text: "How often do you feel tired?", domain: "personal", reverse: false },
    { text: "How often are you physically exhausted?", domain: "personal", reverse: false },
    { text: "How often are you emotionally exhausted?", domain: "personal", reverse: false },
    { text: "How often do you think 'I can't take it anymore'?", domain: "personal", reverse: false },
    { text: "How often do you feel worn out?", domain: "personal", reverse: false },
    { text: "How often do you feel weak and susceptible to illness?", domain: "personal", reverse: false },
    
    { text: "Is your work emotionally exhausting?", domain: "work", reverse: false },
    { text: "Do you feel burnt out because of your work?", domain: "work", reverse: false },
    { text: "Does your work frustrate you?", domain: "work", reverse: false },
    { text: "Do you feel worn out at the end of the working day?", domain: "work", reverse: false },
    { text: "Are you exhausted in the morning at the thought of another day at work?", domain: "work", reverse: false },
    { text: "Do you feel every working hour is tiring for you?", domain: "work", reverse: false },
    { text: "Do you have enough energy for family and friends during leisure time?", domain: "work", reverse: true },

    { text: "Do you find it hard to work with clients?", domain: "client", reverse: false },
    { text: "Do you find it frustrating to work with clients?", domain: "client", reverse: false },
    { text: "Does it drain your energy to work with clients?", domain: "client", reverse: false },
    { text: "Do you feel you give more than you get back when working with clients?", domain: "client", reverse: false },
    { text: "Are you tired of working with clients?", domain: "client", reverse: false },
    { text: "Do you wonder how long you'll be able to continue working with clients?", domain: "client", reverse: false }
  ];

  const optionLabels = [
    { label: "Never / almost never", score: 0 },
    { label: "Seldom", score: 25 },
    { label: "Sometimes", score: 50 },
    { label: "Often", score: 75 },
    { label: "Always", score: 100 }
  ];
  ...
  try {
    const s = localStorage.getItem('burnout_answers');
    const q = localStorage.getItem('burnout_current');
        ...
        <div class="max-w-2xl mx-auto w-full px-4 py-8 sm:px-6 sm:py-12">
          ...
          <div id="quizView" class="space-y-6">
            <div class="flex items-center justify-between border-b border-cream-border pb-4">
              <div>
                <h2 class="font-serif text-2xl text-warm-dark leading-tight">Burnout Assessment (CBI)</h2>
                <p class="text-xs text-warm-gray mt-1">Copenhagen Burnout Inventory</p>
              </div>
              <div class="text-right">
                <span class="text-xs font-medium text-warm-gray" id="questionCounter">Question 1 of 19</span>
                <div class="flex items-center gap-2 mt-1">
                  <div class="progress-track w-24"><div class="progress-fill" id="progressFill" style="width: 5.2%;"></div></div>
                  <span class="text-[10px] font-mono font-medium text-warm-dark" id="progressPercent">5%</span>
                </div>
              </div>
            </div>
            ...
            <div class="card p-5 sm:p-6 mb-6" id="questionCard">
              ...
              <h3 class="font-serif text-lg sm:text-xl text-warm-dark leading-snug mb-6" id="questionText">How often do you feel tired?</h3>
          ...
          <div id="completionView" class="hidden text-center space-y-6 py-12">
            <div class="card p-8 sm:p-10 text-center">
              ...
              <p class="text-sm text-warm-gray mb-8">Thank you for completing the Burnout Assessment. Your results are ready.</p>
          ...
          <div id="resultsView" class="hidden w-full space-y-6">
            <div class="card p-6 sm:p-8 text-center flex flex-col items-center">
              <p class="text-[11px] font-medium text-warm-gray uppercase tracking-wider mb-4">Your Burnout Index</p>
              <div class="relative w-36 h-36 mb-4">
                ...
                <div class="absolute inset-0 flex flex-col items-center justify-center">
                  ...
                  <span class="text-xs text-warm-gray">/ 100</span>
                </div>
              </div>
              <span class="badge badge-low mb-3" id="severityBadge">Low Burnout</span>
              ...
              <div class="mt-5 w-full max-w-xs">
                ...
                <div class="flex justify-between text-[10px] text-warm-gray mt-1">
                  <span>Low (0–24)</span><span>Mild (25-49)</span><span>Mod-High (50-74)</span><span>Severe (75+)</span>
                </div>
              </div>
            </div>
            ...
            <div class="card p-5 sm:p-6">
              ...
              <div id="chartBar">
                <p class="text-xs text-warm-gray mb-3">Score contribution per question (maximum score is 100)</p>
                <div id="barList" class="space-y-3"></div>
              </div>

              <div id="chartRadar" class="hidden">
                <p class="text-xs text-warm-gray mb-3">Visualisation across burnout categories — outer edge = severe (100).</p>
            ...
            <div class="card p-5 sm:p-6">
              ...
              <p class="text-xs text-warm-gray mb-4">CBI measures personal, work-related, and client-related burnout averages.</p>
            ...
            <div class="card p-5 sm:p-6 mb-5">
              ...
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-2">
                <div class="resource-item">
                  ...
                  <div><p class="text-sm font-medium text-warm-dark">Burnout Recovery blueprint</p><p class="text-xs text-warm-gray">Pacing and boundary-setting strategies</p></div>
                </div>
                <div class="resource-item">
                  <div class="resource-icon">
                  ...
                  <div><p class="text-sm font-medium text-warm-dark">Work-Life Balance Guide</p><p class="text-xs text-warm-gray">Boundary setting and recharging techniques</p></div>
                </div>
              </div>
            </div>
            ...
            <div class="card p-5 mb-5" style="background:rgba(232,101,26,.04);">
              <div class="flex gap-3 items-start">
                ...
                <div>
                  <p class="text-xs font-medium text-warm-dark mb-1">About the CBI</p>
                  <p class="text-xs text-warm-gray leading-relaxed">The Copenhagen Burnout Inventory (CBI) was developed by Kristensen et al. (2005). It is a public-domain validated tool focusing on physical, emotional, and situational fatigue. Scores are meant for self-insight and do not replace professional diagnoses.</p>
                </div>
              </div>
            </div>
  ...
  restartBtn.onclick = () => {
    try { localStorage.removeItem('burnout_answers'); localStorage.removeItem('burnout_current'); } catch (e) {}
    renderBurnoutForm();
  };
  ...
  function renderQuestion() {
    ...
    optionLabels.forEach((opt, idx) => {
      const scoreValue = questions[currentQuestion].reverse ? (100 - opt.score) : opt.score;
      const sel = answers[currentQuestion] === scoreValue;
      const div = document.createElement('div');
      div.className = 'option' + (sel ? ' selected' : '');
      div.onclick = () => {
        answers[currentQuestion] = scoreValue;
        try {
          localStorage.setItem('burnout_answers', JSON.stringify(answers));
          localStorage.setItem('burnout_current', String(currentQuestion));
        } catch(e){}
        renderQuestion();
      };
      div.innerHTML = `
  ...
  function getDomainAverages() {
    const personalAnswers = answers.slice(0, 6);
    const workAnswers = answers.slice(6, 13);
    const clientAnswers = answers.slice(13, 19);

    const personal = Math.round(personalAnswers.reduce((a, b) => a + (b || 0), 0) / 6);
    const work = Math.round(workAnswers.reduce((a, b) => a + (b || 0), 0) / 7);
    const client = Math.round(clientAnswers.reduce((a, b) => a + (b || 0), 0) / 6);

    return { personal, work, client };
  }

  function showResults() {
    const { personal, work, client } = getDomainAverages();
    ...
    if (score <= 24) {
      ...
      badge.innerHTML = `<svg width="10" height="10" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7"/></svg> Low Burnout`;
      text.textContent = 'Your responses indicate low burnout risk. You have good emotional and physical reserves. Keep prioritizing wellness!';
      ring.style.stroke = '#5f8a4a';
      setTimeout(() => { document.getElementById('rangeMarker').style.left = (score/24)*24 + '%'; }, 200);
    } else if (score <= 49) {
      level = 'mild';
      badge.className = 'badge badge-moderate mb-3';
      badge.innerHTML = `<svg width="10" height="10" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M12 9v4M12 16h.01"/></svg> Mild Burnout`;
      text.textContent = 'You are experiencing some early signs of burnout. Setting clearer work-life boundaries and scheduled resting times can help.';
      ring.style.stroke = '#e8651a';
      setTimeout(() => { document.getElementById('rangeMarker').style.left = (25 + ((score-25)/24)*24) + '%'; }, 200);
    } else if (score <= 74) {
      level = 'moderate-high';
      badge.className = 'badge badge-high mb-3';
      badge.innerHTML = `<svg width="10" height="10" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M12 9v4M12 16h.01"/></svg> Moderate-High Burnout`;
      text.textContent = 'Elevated burnout levels detected. Consider reducing workload, stepping back temporarily, and practicing pacing.';
      ring.style.stroke = '#c83c28';
      setTimeout(() => { document.getElementById('rangeMarker').style.left = (50 + ((score-50)/24)*24) + '%'; }, 200);
    } else {
      level = 'severe';
      badge.className = 'badge badge-high mb-3';
      badge.innerHTML = `<svg width="10" height="10" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M12 9v4M12 16h.01"/></svg> Severe Burnout`;
      text.textContent = 'High level of burnout indicating severe depletion. We strongly recommend speaking with a counselor or medical professional.';
      ring.style.stroke = '#c83c28';
      setTimeout(() => { document.getElementById('rangeMarker').style.left = (75 + ((score-75)/25)*22) + '%'; }, 200);
    }
  ...
  function buildDomainCards() {
    ...
    document.getElementById('domainCards').innerHTML = `
      <div class="card p-4" style="background:rgba(95,138,74,.04);">
        <p class="text-xs font-medium text-warm-dark">Personal</p>
        <p class="font-serif text-2xl text-warm-dark mt-1">${personal}<span class="text-xs text-warm-gray">/100</span></p>
        <div class="bar-bg mt-2"><div class="bar-fill" style="width:${personal}%;background:#5f8a4a;"></div></div>
      </div>
      <div class="card p-4" style="background:rgba(232,101,26,.04);">
        <p class="text-xs font-medium text-warm-dark">Work-Related</p>
        <p class="font-serif text-2xl text-warm-dark mt-1">${work}<span class="text-xs text-warm-gray">/100</span></p>
        <div class="bar-bg mt-2"><div class="bar-fill" style="width:${work}%;background:#e8651a;"></div></div>
      </div>
      <div class="card p-4" style="background:rgba(200,60,40,.04);">
        <p class="text-xs font-medium text-warm-dark">Client-Related</p>
        <p class="font-serif text-2xl text-warm-dark mt-1">${client}<span class="text-xs text-warm-gray">/100</span></p>
        <div class="bar-bg mt-2"><div class="bar-fill" style="width:${client}%;background:#c83c28;"></div></div>
      </div>
    `;
  }
  ...
  function buildInsights(score) {
    ...
    const { personal, work, client } = getDomainAverages();
    if (personal >= 50) {
      ins.push({ icon: `🔋`, bg: 'rgba(200,60,40,.1)', col: '#c83c28', text: "Your personal exhaustion level is high. Prioritize physical rest and restorative hobbies." });
    }
    if (work >= 50) {
      ins.push({ icon: `💼`, bg: 'rgba(232,101,26,.1)', col: '#e8651a', text: "Work-related stressors are prominent. Discuss boundaries or task distribution with team leaders." });
    }
    if (client >= 50) {
      ins.push({ icon: `👥`, bg: 'rgba(232,101,26,.1)', col: '#e8651a', text: "High client-facing burnout. Schedule buffer periods between client sessions to reset." });
    }
    if (ins.length === 0) {
      ins.push({ icon: `✨`, bg: 'rgba(95,138,74,.1)', col: '#5f8a4a', text: "Keep protecting your energy and maintaining balance!" });
    }
  ...
  function buildResponseReview() {
    const shortQs = [
      "Tiredness", "Physical fatigue", "Emotional fatigue", "Can't take it", "Worn out", "Weak/susceptible",
      "Work exhaustion", "Burnt out at work", "Work frustration", "Day-end exhaustion", "Morning exhaustion", "Tiring work hours", "Energy for social life",
      "Client work difficulty", "Client frustration", "Client drains energy", "Client output/input imbalance", "Tired of clients", "How long can continue"
    ];
    document.getElementById('responseReview').innerHTML = answers.map((raw, idx) => {
      const scoreValue = questions[idx].reverse ? (100 - raw) : raw;
      const opt = optionLabels.find(o => o.score === scoreValue) || { label: "N/A" };
      return `
  ...
  function buildDistChart(userScore) {
    ...
    bins.forEach((b, i) => {
      ...
      ctx.fillStyle=(Math.abs(b.score-userScore)<5)?'#e8651a':'#d8d3c8';
    ...
    [0, 25, 50, 75, 100].forEach(s => {
      const x=pad.left+(s/100)*cW;
      ctx.fillText(s, x, H-6);
    });
    const ux=pad.left+(userScore/100)*cW;
  ...
  function buildRadarChart() {
    ...
    const domains = [
      ...
      { label: "Work-Related", val: work },
      { label: "Client-Related", val: client }
    ];
    ...
    domains.forEach((d, a) => {
      ...
      ctx.fillText(`${d.label} (${d.val})`, lx, ly + 4);
    });
  }
  ...
  function buildBarChart() {
    ...
    answers.forEach((raw, idx) => {
      const pct = raw;
      const cols = ['#5f8a4a', '#ede9df', '#e8a84d', '#e8651a', '#c83c28'];
      const barColor = raw <= 25 ? cols[0] : raw <= 50 ? cols[1] : raw <= 75 ? cols[2] : cols[4];
      const isRev = questions[idx].reverse;
      ...
      row.innerHTML = `
        <div class="flex items-center justify-between mb-1">
          ...
          <span style="font-size:11px;font-family:'JetBrains Mono',monospace;color:#2a2520;">${raw} pts</span>
        </div>
  ...
  function switchTab(tab) {
    ...
    if (tab==='dist')  { setTimeout(() => buildDistChart(getDomainAverages().personal), 50); }
  }
